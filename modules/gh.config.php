 ...
 *  
 * Author: Stackofapps 
 * Author URL: https://www.stackofapps.com 
 * Author Email: admin@stackofapps.com 
 *  
...
class Github_OAuth_Client{ 
    ...
    public function getAccessToken($state, $oauth_code){ 
        $request = $this->tokenURL . '?' . http_build_query([ 
                        'client_id' => $this->clientID, 
                        'client_secret' => $this->clientSecret, 
                        'state' => $state, 
                        'code' => $oauth_code 
                    ]);
        $token = self::apiRequest($request);
    ...
    public function apiRequest($access_token_url){ 
        ...
        $context  = stream_context_create([ 
          'http' => [ 
            'user_agent' => 'Stackofapps GitHub OAuth Login', 
    ...
    public function getAuthenticatedUser($access_token) { 
        ...
        curl_setopt($ch, CURLOPT_USERAGENT, 'Stackofapps GitHub OAuth Login'); 
    ...
    public function getAuthenticatedUserEmail($access_token) { 
